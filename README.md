...
4. 训练结果预测   
训练结果预测需要用到两个文件，分别是fcos.py和predict.py。我们首先需要去fcos.py里面修改model_path以及classes_path，这两个参数必须要修改。   
...
4. 训练结果预测  
训练结果预测需要用到两个文件，分别是fcos.py和predict.py。在fcos.py里面修改model_path以及classes_path。  
**model_path指向训练好的权值文件，在logs文件夹里。  
classes_path指向检测类别所对应的txt。**  
完成修改后就可以运行predict.py进行检测了。运行后输入图片路径即可检测。  
...
### b、使用自己训练的权重
1. 按照训练步骤训练。  
2. 在fcos.py文件里面，在如下部分修改model_path和classes_path使其对应训练好的文件；**model_path对应logs文件夹下面的权值文件，classes_path是model_path对应分的类**。  
```python
_defaults = {
    #--------------------------------------------------------------------------#
...
2. 在fcos.py里面修改model_path以及classes_path。**model_path指向训练好的权值文件，在logs文件夹里。classes_path指向检测类别所对应的txt。**  
...
4. 在fcos.py里面修改model_path以及classes_path。**model_path指向训练好的权值文件，在logs文件夹里。classes_path指向检测类别所对应的txt。**  
